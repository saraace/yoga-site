...
const SingleCategoryScene = ({ scrollY, yVal, duration, startPos, nextStartPos, offsetStyles, height, posterSrc, videoSrc, icon, heading, description, listItems }) => {
    ...
    // intro content
    const introY = useTransform(scrollY, [startPos, startPos+(duration*0.25), startPos+(duration*0.4), startPos+(duration*0.8)], ['50%', '0%', '0%', '-50%']); 
    const introOpacity = useTransform(scrollY, [startPos+(duration*0.4), startPos+(duration*0.8)], [1, 0])

    // benefits content
    const benefitsY = useTransform(scrollY, [startPos+(duration*0.25), startPos+(duration*0.5), startPos+(duration*0.6), startPos+duration], ['50%', '0%', '0%', '-50%']);
